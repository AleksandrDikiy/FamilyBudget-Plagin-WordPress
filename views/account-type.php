<?php
/**
 * Модуль типів рахунків — Family Budget
 *
 * Реалізує довідник системних типів рахунків (Готівка, Картка, Депозит тощо).
...
/**
 * Обробляє POST/GET-запити для CRUD-операцій над типами рахунків.
 *
 ...
 * Перед видаленням перевіряє, чи тип не використовується в активних рахунках.
 *
 * @since  1.0.0
 * @return void
 */
function fb_handle_account_type_actions(): void {
	...
	if ( isset( $_POST['fb_action'] ) && 'save_account_type' === $_POST['fb_action'] ) {
		...
		if ( ! wp_verify_nonce(
			sanitize_key( wp_unslash( $_POST['fb_account_type_nonce'] ?? '' ) ),
			'fb_save_account_type'
		) ) {
			wp_die( esc_html__( 'Помилка безпеки. Оновіть сторінку.', 'family-budget' ) );
		}

		// [SEC-5] wp_unslash() + sanitize_text_field() для рядкових полів.
		$name = sanitize_text_field( wp_unslash( $_POST['account_type_name'] ?? '' ) );

		// [SEC-4] absint() замість intval() для ID.
		$id = absint( wp_unslash( $_POST['account_type_id'] ?? 0 ) );

		if ( $id > 0 ) {
			$wpdb->update(
				$table,
				array( 'AccountType_Name' => $name ),
				array( 'id' => $id ),
				array( '%s' ),
				array( '%d' )
			);
	...
	if ( isset( $_GET['del_account_type'] ) ) {
		...
		$id = absint( wp_unslash( $_GET['del_account_type'] ) );
		...
		$count = (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$wpdb->prefix}Account WHERE AccountType_ID = %d",
				$id
			)
		);
		...
		if ( 0 === $count ) {
			...
			set_transient( 'fb_acc_error', __( 'Неможливо видалити: тип використовується в активних рахунках.', 'family-budget' ), 30 );
		}
	}
}
...
/**
 * Відображає інтерфейс управління типами рахунків.
 *
 ...
 */
function fb_render_account_type_interface(): string {
	...
	$error = get_transient( 'fb_acc_error' );
	if ( $error ) {
	...
	if ( isset( $_GET['edit'] ) ) {
		$edit_item = $wpdb->get_row(
			$wpdb->prepare( "SELECT * FROM {$wpdb->prefix}AccountType WHERE id = %d", absint( wp_unslash( $_GET['edit'] ) ) )
		);
	}

	$types = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}AccountType ORDER BY id ASC" );

	ob_start();
	?>
	<div class="fb-container">
		<h2><i class="dashicons dashicons-bank"></i> <?php esc_html_e( 'Довідник: Типи рахунків', 'family-budget' ); ?></h2>
		...
		<form method="POST" class="fb-card">
			...
			<?php if ( $edit_item ) : ?>
				<input type="hidden" name="account_type_id" value="<?php echo esc_attr( $edit_item->id ); ?>">
			<?php endif; ?>

			<input
				type="text"
				name="account_type_name"
				placeholder="<?php esc_attr_e( 'Назва (напр. Картка, Готівка)', 'family-budget' ); ?>"
				value="<?php echo $edit_item ? esc_attr( $edit_item->AccountType_Name ) : ''; ?>"
				required
			>
		...
		<table class="fb-table">
			<thead>
				<tr>
					...
					<th><?php esc_html_e( 'Назва типу рахунку', 'family-budget' ); ?></th>
			...
			<tbody>
				<?php foreach ( $types as $t ) : ?>
					<tr>
						...
						<td>
							<a href="<?php echo esc_url( add_query_arg( 'edit', $t->id ) ); ?>">📝</a>
